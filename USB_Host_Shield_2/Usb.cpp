...
uint8_t USB::inTransfer(uint8_t addr, uint8_t ep, uint16_t *nbytesptr, uint8_t* data) {
        ...
        if(rcode) {
                USBTRACE3("(USB::InTransfer) SetAddress 11 Failed ", rcode, 0x81);
                USBTRACE3("(USB::InTrans ", rcode, 0x81);
...
uint8_t USB::OutTransfer(EpInfo *pep, uint16_t nak_limit, uint16_t nbytes, uint8_t *data) {
        ...
        while(bytes_left) {
                ...
                Serial.println( "data=");
                for (uint8_t i=0; i < data_p[0]; i++) 
                {
                
                Serial.print( data_p[i]);
                Serial.print( " ");
                } 
