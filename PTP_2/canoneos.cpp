...
uint16_t CanonEOS::SwitchLiveView(bool on)
{
  ...
  //[Jackie] recode 
	if (0)
	{
   Serial.println("Switchliveview on = ");
   Serial.println((on) ? 2 : 0);
   SetProperty(0xD1B0, 2);
   SetProperty(0xD1B3, 0);  	
   delay (5000); 
   //SetProperty(0xD1B0, 2);
   SetProperty(0xD1B8, 4);  //EOS_DPC_VideoRecord 	
   delay (20000); 
	}
	...
	{
    Serial.println("debug_ set property");
    //Serial.println(ptp_error);
...
uint16_t CanonEOS::EventCheck(PTPReadParser *parser)
{
	...
	if ( (ptp_error = Transaction(0x9116, &flags, NULL, parser)) != PTP_RC_OK)
		PTPTRACE2("EOcSEventCheck error:", ptp_error);

	return ptp_error;
}


uint16_t CanonEOS::VideoRecord()
{                           
	uint16_t	ptp_error	= PTP_RC_GeneralError;
	Serial.println( "VideoRecord() Start");      
	ptp_error = SetProperty(0xD1B8, 4);  //EOS_DPC_VideoRecord 
	Serial.println( "VideoRecord() exit");      
	return ptp_error;
}


uint16_t CanonEOS::VideoRecordStop()
{                           
	uint16_t	ptp_error	= PTP_RC_GeneralError;
	Serial.println( "VideoRecordStop() Start");      
	ptp_error = SetProperty(0xD1B8, 0);  //EOS_DPC_VideoRecord 
	Serial.println( "VideoRecordStop() exit");      
	return ptp_error;
}
...
{                           
	...
	Serial.println( "Capture in");      
	//Operation(0x910F, 0, NULL);
	//delay (3000); 
	/*
	 for(uint32_t i = EOS_OC_Capture; i < (EOS_OC_Capture+10); i++) {
	Operation(i, 0, NULL);  
...
*/
  ...
  //Serial.println("0x9125");      
	//Operation(0x9125, 0, NULL);
	//delay (1000); 
	
	    
	        
	//Operation(0x912C, 0, NULL);
	//Operation(0x912D, 0, NULL);
	//Operation(0x912E, 0, NULL);
	//Operation(0x912F, 0, NULL);
	//delay (1000); 
	//PTPTRACE2("2",PTP_RC_GeneralError);
	//Operation(0x9130, 0, NULL);
	//delay (1000);
	//PTPTRACE2("2",PTP_RC_GeneralError);
	//Operation(0x9131, 0, NULL);
	//delay (1000); 
	//PTPTRACE2("2",PTP_RC_GeneralError);
	//Operation(0x9132, 0, NULL);
	//delay (1000);
	//PTPTRACE2("2",PTP_RC_GeneralError);
	//Operation(0x9133, 0, NULL);
	//delay (1000); 
	//PTPTRACE2("2",PTP_RC_GeneralError);
	//Operation(0x9134, 0, NULL);
	//delay (1000);
  //PTPTRACE2("2",PTP_RC_GeneralError);
	...
	Serial.println( "Capture out");      
	//return Operation(0x910F, 0, NULL);
	return Operation(0x910F, 0, NULL);
